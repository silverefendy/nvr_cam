...
class ConfigManager:
    ...
    async def _write_yaml_safe(self, filename: str, data: dict) -> None:
    ...
    async def _create_backup(self, filename: str) -> Path:
    ...
    async def add_camera(self, camera: dict) -> None:
        ...
        await self._create_backup("cameras.yaml")
        config = await self._read_yaml("cameras.yaml")
        ...
        await self._write_yaml_safe("cameras.yaml", config)
    ...
    async def update_system_config(self, data: dict) -> None:
        ...
        await self._create_backup("system.yaml")
        current = await self._read_yaml("system.yaml")
        merged = {**current, **data}
        await self._write_yaml_safe("system.yaml", merged)
    ...
    async def update_storage_config(self, data: dict) -> None:
        """Update storage配置 in storage.yaml."""
        await self._create_backup("storage.yaml")
        await self._write_yaml_safe("storage.yaml", data)
    
    # Notification Configuration
    async def get_notification_config(self) -> dict:
        """Get notification configuration from .env and database."""
        # Read from .env for Telegram settings
    ...
    async def backup_all(self) -> bytes:
        ...
        async with self._lock:
            ...
            with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zipf:
                ...
                # Backup .env (without sensitive data if needed)
                env_file = Path(__file__).parent.parent.parent / ".env"
                if env_file.exists():
