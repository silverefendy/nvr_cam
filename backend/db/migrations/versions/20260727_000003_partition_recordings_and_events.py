...
Notes:
  - Semua fungsi helper (is_partitioned, table_exists) menggunakan
    synchronous connection dari op.get_bind() — ini aman karena Alembic
    migration 003 berjalan di dalam run_sync() di env.py.
  - DROP TABLE pakai CASCADE untuk avoid sequence dependency error.
"""
...
def upgrade() -> None:
    ...
    # Detect existing state and skip steps that already completed.
    ...
    # 3. Create partitioned recordings table (skip if already created)
    if not table_exists("recordings"):
        ...
        # Monthly partitions for recordings (2025–2027)
        for year in [2025, 2026, 2027]:
            for month in range(1, 13):
                partition_name = f"recordings_y{year}m{month:02d}"
                start_date = f"{year}-{month:02d}-01"
                end_date = f"{year + 1}-01-01" if month == 12 else f"{year}-{month + 1:02d}-01"
                op.execute(f"CREATE TABLE {partition_name} PARTITION OF recordings "
        ...
        op.execute("""
            INSERT INTO recordings
                (id, camera_id, file_path, file_size_mb, started_at, ended_at,
                 duration_s, codec, is_protected, is_encoded_av1, created_at)
            SELECT id, camera_id, file_path, file_size_mb, started_at, ended_at,
                   duration_s, codec, is_protected, is_encoded_av1, created_at
            FROM recordings_old;
        """)

    # 4. Create partitioned motion_events table (skip if already created)
    if not table_exists("motion_events"):
        ...
        # Monthly partitions for motion_events (2025–2027)
        for year in [2025, 2026, 2027]:
            for month in range(1, 13):
                partition_name = f"motion_events_y{year}m{month:02d}"
                start_date = f"{year}-{month:02d}-01"
                end_date = f"{year + 1}-01-01" if month == 12 else f"{year}-{month + 1:02d}-01"
                op.execute(f"CREATE TABLE {partition_name} PARTITION OF motion_events "
                           f"FOR VALUES FROM ('{start_date}') TO ('{end_date}')")
        ...
        op.execute("""
            INSERT INTO motion_events
                (id, camera_id, recording_id, zone_name, started_at, ended_at,
                 duration_s, snapshot_path, video_offset_s, severity, notified, created_at)
            SELECT id, camera_id, recording_id, zone_name, started_at, ended_at,
                   duration_s, snapshot_path, video_offset_s, severity, notified, created_at
            FROM motion_events_old;
        """)

    # 5. Detach sequences from old tables, then drop them (CASCADE is safe here).
    #    ALTER SEQUENCE ... OWNED BY NONE prevents the sequence from being
    #    dropped together with the table; the partitioned tables reference the
    #    sequences via DEFAULT nextval(...) which is sufficient.
    ...
    # 6. Create indexes on the new partitioned tables (skip if already exist)
    if not index_exists("recordings", "ix_recordings_camera_id"):
        op.create_index("ix_recordings_camera_id", "recordings", ["camera_id"])
    if not index_exists("recordings", "ix_recordings_started_at"):
        op.create_index("ix_recordings_started_at", "recordings", ["started_at"])

    if not index_exists("motion_events", "ix_motion_events_camera_id"):
        op.create_index("ix_motion_events_camera_id", "motion_events", ["camera_id"])
    if not index_exists("motion_events", "ix_motion_events_started_at"):
...
def downgrade() -> None:
    ...
    op.execute("""
        ...
        SELECT id, camera_id, recording_id, zone_name, started_at, ended_at,
               duration_s, snapshot_path, video_offset_s, severity, notified, created_at
        FROM motion_events_part;
    """)

    op.execute("ALTER SEQUENCE motion_events_id_seq OWNED BY NONE;")
    op.execute("ALTER SEQUENCE recordings_id_seq OWNED BY NONE;")
    op.execute("DROP TABLE IF EXISTS motion_events_part CASCADE;")
    op.execute("DROP TABLE IF EXISTS recordings_part CASCADE;")
