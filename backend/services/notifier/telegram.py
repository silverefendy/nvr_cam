...
class TelegramNotifier:
    """Wrapper class for Telegram notification functions."""
    ...
    async def notify_motion(camera_name: str, zone_name: str, snapshot_path: str | None):
        ...
        if snapshot_path and Path(snapshot_path).exists():
            await TelegramNotifier.send_photo(snapshot_path, caption=text)
        else:
            await TelegramNotifier.send_message(text)

    @staticmethod
    async def notify_camera_offline(camera_name: str):
        await TelegramNotifier.send_message(f"⚠️ <b>KAMERA OFFLINE</b>\n📷 {camera_name}\nSegera periksa koneksi.")

    @staticmethod
    async def notify_disk_warning(drive: str, free_pct: float):
        await TelegramNotifier.send_message(f"💾 <b>DISK HAMPIR PENUH</b>\nDrive: {drive}\nSisa: {free_pct:.1f}%")


# Keep standalone functions for backward compatibility
async def send_message(text: str, chat_id: str | None = None) -> bool:
    return await TelegramNotifier.send_message(text, chat_id)

async def send_photo(photo_path: str, caption: str, chat_id: str | None = None) -> bool:
    return await TelegramNotifier.send_photo(photo_path, caption, chat_id)

async def notify_motion(camera_name: str, zone_name: str, snapshot_path: str | None):
    await TelegramNotifier.notify_motion(camera_name, zone_name, snapshot_path)

async def notify_camera_offline(camera_name: str):
    await TelegramNotifier.notify_camera_offline(camera_name)

async def notify_disk_warning(drive: str, free_pct: float):
    await TelegramNotifier.notify_disk_warning(drive, free_pct)
